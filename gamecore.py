...
print (" The Objective is guess the hidden word by suggesting letters before you run out of chances. The player guesses one letter at a time. If the letter is in the word: All matching positions are revealed, else one attempt is lost. The player wins by guessing all letters in the word before attempts run out. The player loses if all allowed wrong guesses are used before the word is complete. All words are 6 letter and you will get only 3 hints")
print input()
